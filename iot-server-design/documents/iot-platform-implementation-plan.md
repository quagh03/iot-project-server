...
| **5** Heartbeat / Health / Connectivity | ⛔ **NOT STARTED** | No heartbeat ingest, no LWT consumption, no `device_health` upsert path. `GET /v1/connectivity` and the read-side `HealthService` shipped early with Phase 4; `GET /v1/devices/{deviceId}/health` still pending. |
