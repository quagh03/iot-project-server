...
class AuditServiceIT {
    ...
    void appends_entry_to_partitioned_audit_logs() {
        audit.user("usr_8f3a", "user.login", "/v1/auth/login",
                Map.of("ua", "test-agent"), "127.0.0.1");

        Integer count = jdbc.queryForObject(
                "SELECT count(*)::int FROM audit_logs WHERE actor = 'usr_8f3a' AND event = 'user.login'",
                Integer.class);
    ...
    void system_appends_use_system_actor_type() {
        audit.system("partition.created", "telemetry_2099_01",
                Map.of("table", "telemetry", "month", "2099-01"));

        String actorType = jdbc.queryForObject(
                "SELECT actor_type FROM audit_logs WHERE event = 'partition.created' LIMIT 1",
                String.class);
